...
import {capitalWithdrawPendingManage,AcceptCapitalWithdraw,RejectCapitalWithdraw } from '../../../store/withdrawalSlice';

const PendingCapitalWithdraw = () => {
  const dispatch = useDispatch();
  const { data } = useSelector((state) => state.capitalWithdrawPendingManageReducer);
  const { data: acceptData } = useSelector((state) => state.AcceptCapitalWithdrawReducer);
  const { data: rejectData } = useSelector((state) => state.RejectCapitalWithdrawReducer);
  ...
  useEffect(() => {
    dispatch(capitalWithdrawPendingManage());
  ...
  const rejectHandler=(id)=>{
    dispatch(RejectCapitalWithdraw(id))
  }
  ...
  const handleConfirmAcceptance = () => {
    dispatch(AcceptCapitalWithdraw(selectedPackageId));
    ...
    <Card>
      <CardBody>
        <div className="d-flex align-items-center justify-content-between">
          <div>
            <CardTitle tag="h4">Capital Withdraw Pending</CardTitle>
          </div>
          ...
          <tbody>
            {data && data.userData.map((tdata, index) => (
              <tr key={tdata.email} className="border-top">
                ...
                <td>{tdata.phone}</td>
                <td>{tdata.withdrawAmount}</td>
                <td>{tdata.capitalWithdrawUrl}</td>
...
export default PendingCapitalWithdraw;
