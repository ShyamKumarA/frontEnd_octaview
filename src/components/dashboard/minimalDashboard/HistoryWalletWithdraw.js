...
import { walletWithdrawHistoryManage} from '../../../store/reportSlice';


const HistoryAddFund = () => {
  const dispatch = useDispatch();
  const { data } = useSelector((state) => state.walletWithdrawHistoryManageReducer);
  

  useEffect(() => {
    dispatch(walletWithdrawHistoryManage());
  }, [dispatch])
    ...
    <Card>
      <CardBody>
        <div className="d-flex align-items-center justify-content-between">
          <div>
            <CardTitle tag="h4">Wallet Withdraw History</CardTitle>
          </div>
