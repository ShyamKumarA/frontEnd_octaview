...
import { capitalWithdrawHistoryManage} from '../../../store/reportSlice';


const HistoryAddFund = () => {
  const dispatch = useDispatch();
  const { data } = useSelector((state) => state.capitalWithdrawHistoryManageReducer);
  

  useEffect(() => {
    dispatch(capitalWithdrawHistoryManage());
  }, [dispatch])
    ...
    <Card>
      <CardBody>
        <div className="d-flex align-items-center justify-content-between">
          <div>
            <CardTitle tag="h4">Capital Withdraw History</CardTitle>
          </div>
